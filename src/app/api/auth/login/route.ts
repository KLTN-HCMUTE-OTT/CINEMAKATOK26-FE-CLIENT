...
export async function POST(req: Request) {
  try {
    const payload = await readJsonBody<LoginBody>(req);

    const backendRes = await fetch(backendAuthUrl("/api/v1/auth/login"), {
      method: "POST",
    ...
    if (!backendRes.ok) {
      const message = await parseBackendError(backendRes, "Login failed");
